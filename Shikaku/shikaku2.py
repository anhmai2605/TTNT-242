...
class PuzzleSolver:
    ...
    def initialize_factors(self) -> None:
        ...
        # Add print for factors
        print("\nFactors for each value:")
        for i, (_, _, value) in enumerate(self.data.location_data):
            print(f"  Value {value}: {self.data.factors_cache[i]}")
    ...
    def set_region_value(L: List[List[int]], r1: int, r2: int, c1: int, c2: int, 
        ...
        for r in range(r1, r2 + 1):
            for c in range(c1, c2 + 1):
                L[r][c] = value
        # Print added after setting region (outside method to preserve original signature)
    ...
    def dfs(self, next_index: int) -> bool:
            ...
            for i in range(e_value // fac):
                for j in range(fac):
                    ...
                    if (self.is_valid_bounds(self.data.state, r1, r2, c1, c2) and
                        ...
                        # Print after setting region
                        print(f"Placing rectangle ({r1}, {c1}) to ({r2}, {c2}) for index {next_index}")
                        self.print_grid(self.data.state)
                        ...
                        print(f"Backtracking on ({r1}, {c1}) to ({r2}, {c2})")
        ...
        # Print when no valid placement is found
        print(f"No valid placement for index {next_index}, backtracking...")
    ...
    def initialize(self) -> None:
        ...
        # Print initial state
        print("\nInitial state with marked starting positions:")
        self.print_grid(self.data.state)
        ...
        # Print possible cells for each region
        print("\nPossible cells for each region:")
        for i, cells in enumerate(self.data.last_cells):
            print(f"  Region {i}: {cells}")
